...
public class FragmentMyTask extends Fragment {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        databaseReference = FirebaseDatabase.getInstance().getReference().child("requestedTasks");
        databaseReference2 = FirebaseDatabase.getInstance().getReference().child("WorkerProfile");
        databaseReference2.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                id = Integer.parseInt(dataSnapshot.child(username).child("speciality").getValue()+"");
            }
        ...
        databaseReference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for (DataSnapshot data : dataSnapshot.getChildren()) {
                    ...
                    //Toast.makeText(getContext(), id + "\n" + data.child("job_id").getValue(), Toast.LENGTH_SHORT).show();
                    if (data.child("job_id").getValue().toString().equals(id + "")) {
